...
Active values are equivalence classes, under translation in time, of
"concretely scheduled" time-varying values.  Every Active comes with
an "era", a *finite* duration used for aligning, sequencing, and so
on.  However, Active values may be defined outside their era.

In fact, we require that Active values are always defined
*everywhere*.  Note the fact that Actives can be defined outside their
era also requires that in order to sequence two Active values, we must
have a way to combine their values.  So we also require a semigroup
structure on the underlying values when sequencing. I used to think
these were big restriction---what if one wants partial values,
i.e. one can define an Active value only for a certain era and not
outside it?  And what if one wants to build Actives over values that
do not form a semigroup?

The latter is easily answered: one may always use one of the two
trivial semigroup structures x <> y = x or x <> y = y.  The former, in
turn, is not actually a big restriction either: one can always adjoin
an extra element a la Maybe, making it either an identity (treating it
as a "default value" which is "overridden" by other values) or as an
annihilator (treating it like an infectious "undefined" value).

An era is simply a finite duration (possibly negative!).  We only
consider finite eras: we can still have active values defined over an
infinite extent.

Somewhat unfortunately this is looking more and more like Janin et
al.---though I think there are still plenty of new things to say,
e.g. re: the 2-category structure, equivalence classes under
translation, the relationship to other related work such as Hudak's
temporal media, and simply giving a reasoned account of WHY this is
the right design.

There may also be a nice story to tell about retaining more a more
intensional representation of Actives as compositions of more
primitive things, using the 2-category structure to give a canonical
representation, to enable more efficient compilation (though I think a
lot of that has already been covered by Hudak).
...
-- |
-- Module      :  Data.Active
-- Copyright   :  (c) 2013 Andy Gill, Nick Wu, Brent Yorgey
...
newtype Era t = Era { _eraDuration :: Diff t }

makeLenses ''Era

unionEras :: Ord (Diff t) => Era t -> Era t -> Era t
unionEras (Era d1) (Era d2) = Era (d1 `max` d2)

intersectEras :: Ord (Diff t) => Era t -> Era t -> Era t
intersectEras (Era d1) (Era d2) = Era (d1 `min` d2)
...
mkActive :: AffineSpace t => t -> t -> (t -> a) -> Active t a
mkActive s e f = Active (Era (e .-. s)) (f . (s.+^))
...
(..$) :: AffineSpace t => t -> t -> (t -> a) -> Active t a
(..$) = mkActive

-- | Create a constant 'Active' value.
pureA :: a -> Active t a
pureA = PureA

-- | \"Apply\" an 'Active' function to an 'Active' value, aligning
--   them at their start times and taking the union of their
--   intervals.  This is simply a default, XXX.
appA :: (AffineSpace t, Ord (Diff t)) => Active t (a -> b) -> Active t a -> Active t b
appA (PureA f     ) (PureA x     ) = PureA (f x)
appA (PureA f     ) (Active ex x ) = Active ex (f <$> x)
appA (Active ef f ) (PureA x     ) = Active ef (($ x) <$> f)
appA (Active ef f ) (Active ex x ) = Active (unionEras ef ex) (f <*> x)

instance (Ord (Diff t), AffineSpace t) => Applicative (Active t) where
  pure  = pureA
  (<*>) = appA
